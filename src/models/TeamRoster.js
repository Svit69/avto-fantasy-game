...
export class TeamRoster {
  ...
  markConfirmed() { this.#lifecycle.markConfirmed(); this.#syncSlotEditability(); }
  markEditing() { this.#lifecycle.markEditing(); this.#syncSlotEditability(); }
  ...
  assignPlayerSelectionAt(slotIndex, player) { if (this.isEditable() && this.canSelectPlayerFromClub(slotIndex, player)) this.getSlotByIndex(slotIndex)?.assignPlayerSelection(player); }
  clearPlayerSelectionAt(slotIndex) { if (this.isEditable()) this.getSlotByIndex(slotIndex)?.clearPlayerSelection(); }
  #syncSlotEditability() { this.#slots.forEach((slot) => slot.setEditable(this.isEditable())); }
}
